...
/* ================================================================
   ...
   Renders:
     ...
     • buildCategoryView(file, cat, generatedAt, showPreview, client, validationMode, inputSource, emailMetadata)
...
var Renderer = (function () {
    ...
    var CATEGORY_META = {
        ...
        EMAIL_METADATA:  { icon: '&#9993;',  title: 'Email Metadata',                       desc: 'Shows the inbox metadata captured from the post-send email selected for validation.' },
    ...
    ============================================================ */
    function buildCategoryView(file, categoryKey, generatedAt, showPreview, client, validationMode, inputSource, emailMetadata) {
        if (categoryKey === 'overview') {
            return _buildOverview(file, showPreview, client, validationMode, inputSource);
        }
        ...
        if (categoryKey === 'EMAIL_METADATA') {
            return _buildEmailMetadataAudit(emailMetadata);
        }
    ...
    ============================================================ */
    function _buildOverview(file, showPreview, client, validationMode, inputSource) {
        var rules        = Array.isArray(file.rules) ? file.rules : [];
        var clientName   = client || 'General';
        var modeName     = validationMode || 'PRE_SEND';
        var inputName    = inputSource || 'HTML Folder';
        ...
        var statsHtml =
            ...
            _tile('Validation Mode', modeName,    'workflow',        'tile-total',   '&#128203;') +
            _tile('Input Source',  inputName,     'source',          'tile-total',   '&#128229;') +
    ...
    /* ============================================================
       EMAIL METADATA
    ============================================================ */
    function _buildEmailMetadataAudit(metadata) {
        metadata = metadata || {};
        var available = !!metadata.available;

        var statsHtml =
            '<div class="section-label">Email Metadata Summary</div>' +
            '<div class="summary-strip strip-5">' +
            _tile('Source', available ? 'Gmail' : 'Pre-Send', available ? 'inbox email' : 'local HTML', 'tile-total', '&#128229;') +
            _tile('Subject', available && metadata.subject ? 'Captured' : 'N/A', 'message subject', available && metadata.subject ? 'tile-passed' : 'tile-skipped', '&#9993;') +
            _tile('Sender', available && metadata.from ? 'Captured' : 'N/A', 'from address', available && metadata.from ? 'tile-passed' : 'tile-skipped', '&#128100;') +
            _tile('Recipients', available && _metadataList(metadata.to) ? 'Captured' : 'N/A', 'to address', available && _metadataList(metadata.to) ? 'tile-passed' : 'tile-skipped', '&#128101;') +
            _tile('Received Date', available && metadata.receivedDate ? 'Captured' : 'N/A', 'inbox timestamp', available && metadata.receivedDate ? 'tile-passed' : 'tile-skipped', '&#128197;') +
            '</div>';
        ...
        if (!available) {
            return (
                '<div class="category-header">' +
                '<div class="category-eyebrow">AUDIT CATEGORY</div>' +
                '<h1 class="category-title">&#9993; Email Metadata</h1>' +
                '<p class="category-description">Shows inbox metadata captured from the post-send email selected for validation.</p>' +
                '</div>' +
                statsHtml +
                '<div class="no-results">' +
                '<div class="no-results-icon">&#9993;</div>' +
                '<div class="no-results-title">No post-send email metadata available.</div>' +
                '<div class="no-results-sub">Pre-Send audits use local HTML files, so inbox sender and delivery metadata is not collected.</div>' +
                '</div>'
            );
        }
        ...
        var rows = [
            ...
            ['From', metadata.from],
            ['To', _metadataList(metadata.to)],
            ['Reply-To', metadata.replyTo],
            ['CC', _metadataList(metadata.cc)],
            ['BCC', _metadataList(metadata.bcc)],
            ['Received Date', metadata.receivedDate ? Utils.formatDate(metadata.receivedDate) : ''],
        ...
        return (
            ...
            '</div>' +
            statsHtml +
            '<div class="section-label">Inbox Details</div>' +
            '<div class="links-panel">' +
            '<div class="links-table-wrap">' +
            '<table class="links-table email-metadata-table">' +
            '<tbody>' + rows.map(_buildEmailMetadataRow).join('') + '</tbody>' +
            '</table>' +
            '</div>' +
            '</div>'
        );
    ...
    function _buildEmailMetadataRow(row) {
        return (
            '<tr>' +
            '<th scope="row" class="links-cell-text email-metadata-label">' + _esc(row[0]) + '</th>' +
            '<td class="links-cell-text email-metadata-value">' + _esc(row[1] || 'N/A') + '</td>' +
            '</tr>'
        );
    }

    function _metadataList(values) {
        return Array.isArray(values)
            ? values.filter(function (value) { return String(value || '').trim(); }).join(', ')
            : '';
    }
